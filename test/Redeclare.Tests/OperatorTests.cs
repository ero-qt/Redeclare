...
public sealed class OperatorTests
{
    ...
    private static string[] Names => [.. Compilation.Type("V").ToTypeDeclaration().Members.OfType<MethodDeclaration>().Select(m => m.Name.ToString())];
    ...
    public void Render_ExplicitInterfaceOperators_NameTheInterface()
    {
        var compilation = Compiling.AssertCompiles("""
            ...
            """);
        var w = compilation.Type("W").ToTypeDeclaration();
        var withBodies = w with { Members = [.. w.Members.Select(m => m is MethodDeclaration method ? method with { Body = Snippet.Expression(method.Name is MethodName.Operator { Token: "+", IsChecked: false } ? "a" : "0") } : m)] };
    ...
    {
        var members = Compilation.Type("V").ToTypeDeclaration().Members.OfType<MethodDeclaration>().ToList();

        using (Assert.EnterMultipleScope())
        {
            Assert.That(members.Single(m => m.Name == new MethodName.Operator("+=")).Modifiers, Is.EqualTo(Modifiers.None));
            Assert.That(members.Single(m => m.Name == new MethodName.Operator("+", IsChecked: true)).Modifiers, Is.EqualTo(Modifiers.Static));
            Assert.That(members.Single(m => m.Name is MethodName.Conversion { IsImplicit: true }).ReturnType, Is.EqualTo(Types.Int32));
        }
    }
    ...
    {
        var type = Compilation.Type("V").ToTypeDeclaration();
        var withBodies = type with
        {
            Members = [.. type.Members.Select(m => m switch
            {
                ...
                MethodDeclaration { Name: MethodName.Conversion } method
